...
async function loadHero() {
    ...
    try {
        ...
        let titleToUse = settings.homepage_hero_title || settings.hero_title;
        let descToUse = settings.homepage_hero_desc || settings.hero_description;

        if (window.location.pathname.includes('articles.html')) {
            // Default to static HTML content if no custom setting
            titleToUse = null;
            descToUse = null;
            ...
            // Apply Database Settings for About Page
            titleToUse = null; // Default null to keep static if no setting
            descToUse = null;
            ...
            if (aboutUsDescEl && settings.about_us_desc) {
                // Simple markdown-ish: Newlines to <p>
        ...
        if (titleToUse) {
            // Update title globally or target specific IDs
            const heroTitle = document.getElementById('hero-title') || document.getElementById('articles-hero-title');
            if (heroTitle) {
                // Format title: If contains '|', color the second part
                if (titleToUse.includes('|')) {
                    const parts = titleToUse.split('|');
                    heroTitle.innerHTML = `${parts[0]} <span>${parts[1]}</span>`;
                } else {
                    heroTitle.innerText = titleToUse;
                }
            }

            if (descToUse) {
                const heroDesc = document.getElementById('hero-description') || document.getElementById('articles-hero-description');
                if (heroDesc) heroDesc.innerText = descToUse;
            }
        }
        ...
        // If no slides, use a placeholder or handle gracefully
        if (slides.length === 0) {
        ...
        // Start Loop if more than 1 slide
        if (slides.length > 1) {
    ...
    try {
        ...
        if (settings.auth_login_text) {
            document.querySelectorAll('.btn-login').forEach(btn => {
                if (btn.innerText !== 'Ã‡Ä±kÄ±ÅŸ') btn.innerText = settings.auth_login_text;
            });
        }
        ...
        if (navMenu) {
            ...
            menus.forEach(menu => {
                ...
                if (isCategoryMenu && categories.length > 0) {
                    ...
                    if (menu.label === 'HakkÄ±nda') menu.url = 'about.html'; // Override for About page
                    ...
                    // "TÃ¼mÃ¼" (All) link removed as per user request
                    // Categories only
        ...
        if (mobileMenu) {
            ...
            menus.forEach(menu => {
                ...
                if (isCategoryMenu && categories.length > 0) {
                    ...
                    // "TÃ¼mÃ¼" removed for mobile as well
    ...
    signupForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                showToast('KayÄ±t baÅŸarÄ±lÄ±! LÃ¼tfen giriÅŸ yapÄ±n.', 'success');
    ...
    if (user) {
        authButtons.forEach(container => {
            ...
            if (user.role === 'admin') {
                ...
                roleBadge = '<span class="role-badge editor">EditÃ¶r</span>';
            }
            ...
            container.innerHTML = `
                ...
                <button class="btn btn-login btn-sm" onclick="logout()">Ã‡Ä±kÄ±ÅŸ</button>
            `;
        ...
        authButtons.forEach(container => {
            container.innerHTML = `
                <button class="btn btn-login" onclick="openModal('loginModal')">GiriÅŸ Yap</button>
                <button class="btn btn-signup" onclick="openModal('signupModal')">KayÄ±t Ol</button>
            `;
        });
...
async function loadShowcase() {
    try {
        ...
        if (!settingsRes.ok || !articlesRes.ok) return;
...
document.addEventListener('click', function (e) {
    if (e.target && e.target.id === 'hero-main-btn') {
        e.preventDefault();
...
if (newsletterForm) {
    newsletterForm.addEventListener('submit', async (e) => {
        ...
        try {
            btn.disabled = true;
            btn.innerHTML = '<i class="ph ph-spinner ph-spin"></i> Ä°ÅŸleniyor...';
            ...
            if (res.ok) {
                showToast(data.message || 'TeÅŸekkÃ¼rler, kaydÄ±nÄ±z alÄ±ndÄ±!', 'success');
                emailInput.value = '';
            } else {
...
async function loadArticlesPage() {
    ...
    try {
        ...
        grid.innerHTML = `<div style="grid-column:1/-1; text-align:center; padding:40px;">Hata oluÅŸtu: ${escapeHtml(e.message)}</div>`;
    }
}
...
function renderArticlesGrid() {
    ...
    if (filteredArticles.length === 0) {
        grid.innerHTML = `<div style="grid-column:1/-1; text-align:center; padding:60px; color:#64748b; font-size:1.1rem;">AradÄ±ÄŸÄ±nÄ±z kriterlere uygun makale bulunamadÄ±.</div>`;
...
async function loadArticleDetail() {
    ...
    try {
        ...
        if (!res.ok) throw new Error('Makale bulunamadÄ±');
        ...
        console.error(err);
        showToast('Makale yÃ¼klenirken hata oluÅŸtu.', 'error');
        setTimeout(() => {
...
async function loadFrontendCategories() {
    ...
    try {
        ...
        if (setRes.ok) {
            ...
            const heroTitle = document.getElementById('articles-hero-title');
            const heroDesc = document.getElementById('articles-hero-description');
            if (heroTitle && settings.articles_hero_title) heroTitle.innerHTML = settings.articles_hero_title.replace(' ', '<span>') + '</span>'; // Simple Span inject for style if needed or just text
            // The HTML has `Maka<span>leler</span>`, let's just replace text but keep span if possible? 
            // The user input is plain text. Let's just set innerText or simple HTML if we want the span effect.
            // If the user types "Makaleler", it replaces the whole thing. 
            // If we want to keep the styling span, we'd need a robust way. 
            // For now, let's just set the text. The user can't easily add span via input unless we allow HTML.
            // But let's check `articles.html`. It has `Maka<span>leler</span>`.
            // Let's just set innerText. If they want style, they lose the span for now.
            if (heroTitle && settings.articles_hero_title) heroTitle.innerText = settings.articles_hero_title;
            if (heroDesc && settings.articles_hero_desc) heroDesc.innerText = settings.articles_hero_desc;
...
 
 / /   - - -   A r t i c l e   I n t e r a c t i o n s   ( L i k e s   &   C o m m e n t s )   - - - 
 
 
 
 a s y n c   f u n c t i o n   l o a d L i k e s ( i d )   { 
 
         t r y   { 
 
                 c o n s t   t o k e n   =   l o c a l S t o r a g e . g e t I t e m ( ' t o k e n ' ) ; 
 
                 c o n s t   h e a d e r s   =   t o k e n   ?   {   ' A u t h o r i z a t i o n ' :   ` B e a r e r   $ { t o k e n } `   }   :   { } ; 
 
                 c o n s t   r e s   =   a w a i t   f e t c h ( ` $ { A P I _ U R L } / a r t i c l e s / $ { i d } / l i k e ` ,   {   h e a d e r s   } ) ; 
 
                 c o n s t   d a t a   =   a w a i t   r e s . j s o n ( ) ; 
 
 
 
                 c o n s t   b t n   =   d o c u m e n t . g e t E l e m e n t B y I d ( ' l i k e - b t n ' ) ; 
 
                 i f   ( ! b t n )   r e t u r n ; 
 
 
 
                 i f   ( d a t a . l i k e d )   { 
 
                         b t n . c l a s s L i s t . a d d ( ' l i k e d ' ) ; 
 
                         b t n . i n n e r H T M L   =   ` < i   c l a s s = " p h - f i l l   p h - h e a r t " > < / i >   B e Ä xe n d i n   &   D e s t e k   O l d u n ` ; 
 
                 }   e l s e   { 
 
                         b t n . i n n e r H T M L   =   ` < i   c l a s s = " p h   p h - h e a r t " > < / i >   B e Ä xe n   &   D e s t e k   O l ` ; 
 
                 } 
 
         }   c a t c h   ( e )   {   c o n s o l e . e r r o r ( ' L i k e   l o a d ' ,   e ) ;   } 
 
 } 
 
 
 
 a s y n c   f u n c t i o n   t o g g l e L i k e ( )   { 
 
         c o n s t   t o k e n   =   l o c a l S t o r a g e . g e t I t e m ( ' t o k e n ' ) ; 
 
         i f   ( ! t o k e n )   { 
 
                 o p e n M o d a l ( ' l o g i n M o d a l ' ) ; 
 
                 r e t u r n ; 
 
         } 
...
         t r y   { 
 
                 / /   O p t i m i s t i c   U I 
 
                 c o n s t   i s L i k e d   =   b t n . c l a s s L i s t . c o n t a i n s ( ' l i k e d ' ) ; 
 
 
 
                 i f   ( i s L i k e d )   { 
 
                         b t n . c l a s s L i s t . r e m o v e ( ' l i k e d ' ) ; 
 
                         b t n . i n n e r H T M L   =   ` < i   c l a s s = " p h   p h - h e a r t " > < / i >   B e Ä xe n   &   D e s t e k   O l ` ; 
 
                 }   e l s e   { 
...
 
 
                 c o n s t   r e s   =   a w a i t   f e t c h ( ` $ { A P I _ U R L } / a r t i c l e s / $ { i d } / l i k e ` ,   { 
 
                         m e t h o d :   ' P O S T ' , 
 
...
                 i f   ( ! r e s . o k )   { 
 
                         l o a d L i k e s ( i d ) ;   / /   R e v e r t   o n   e r r o r 
 
                 } 
 
...
 
 
 a s y n c   f u n c t i o n   l o a d C o m m e n t s ( i d )   { 
 
         t r y   { 
 
                 c o n s t   t o k e n   =   l o c a l S t o r a g e . g e t I t e m ( ' t o k e n ' ) ; 
...
 
 
                 c o n s t   r e s   =   a w a i t   f e t c h ( ` $ { A P I _ U R L } / a r t i c l e s / $ { i d } / c o m m e n t s ` ,   {   h e a d e r s   } ) ; 
 
                 i f ( ! r e s . o k )   r e t u r n ; 
 
                 c o n s t   c o m m e n t s   =   a w a i t   r e s . j s o n ( ) ; 
 
 
 
                 c o n s t   c o u n t E l   =   d o c u m e n t . g e t E l e m e n t B y I d ( ' c o m m e n t - c o u n t ' ) ; 
 
                 i f ( c o u n t E l )   c o u n t E l . i n n e r T e x t   =   c o m m e n t s . l e n g t h ; 
 
                 
 
                 c o n s t   l i s t   =   d o c u m e n t . g e t E l e m e n t B y I d ( ' c o m m e n t s - l i s t ' ) ; 
 
...
 
                         l i s t . i n n e r H T M L   =   ' < d i v   c l a s s = " c o m m e n t - i t e m   p l a c e h o l d e r "   s t y l e = " j u s t i f y - c o n t e n t : c e n t e r ;   c o l o r : # 9 4 a 3 b 8 ;   f o n t - s t y l e : i t a l i c ; " > H e n Ã ¼ z   y o r u m   y a p Ä ± l m a m Ä ± Å x.   Ä ° l k   y o r u m u   s i z   y a p Ä ± n ! < / d i v > ' ; 
 
                 }   e l s e   { 
 
                         l i s t . i n n e r H T M L   =   c o m m e n t s . m a p ( c   = >   { 
 
                                 c o n s t   i n i t i a l s   =   c . f u l l n a m e . s p l i t ( '   ' ) . m a p ( n   = >   n [ 0 ] ) . j o i n ( ' ' ) . s u b s t r i n g ( 0 ,   2 ) . t o U p p e r C a s e ( ) ; 
 
                                 r e t u r n   ` 
 
                                 < d i v   c l a s s = " c o m m e n t - i t e m "   i d = " c o m m e n t - $ { c . i d } " > 
 
                                         < d i v   c l a s s = " c o m m e n t - a v a t a r " > $ { i n i t i a l s } < / d i v > 
 
                                         < d i v   c l a s s = " c o m m e n t - c o n t e n t - w r a p p e r " > 
 
                                                 < d i v   c l a s s = " c o m m e n t - h e a d e r " > 
 
                                                         < s p a n   c l a s s = " c o m m e n t - a u t h o r " > $ { e s c a p e H t m l ( c . f u l l n a m e ) } < / s p a n > 
 
                                                         $ { c . i s _ a p p r o v e d   = = =   0   ?   ' < s p a n   c l a s s = " c o m m e n t - s t a t u s - p e n d i n g " > < i   c l a s s = " p h   p h - c l o c k "   s t y l e = " m a r g i n - r i g h t : 4 p x ; " > < / i >   O n a y   B e k l i y o r < / s p a n > '   :   ' ' } 
 
                                                         < d i v   c l a s s = " c o m m e n t - a c t i o n s " > 
 
                                                                 < s p a n   c l a s s = " c o m m e n t - d a t e " > $ { n e w   D a t e ( c . c r e a t e d _ a t ) . t o L o c a l e D a t e S t r i n g ( ' t r - T R ' ) } < / s p a n > 
 
                                                                 $ { c . i s _ m i n e   ?   ` 
 
                                                                         < s p a n   s t y l e = " m a r g i n :   0   8 p x ;   c o l o r : # c b d 5 e 1 ; " > | < / s p a n > 
 
                                                                         < s p a n   c l a s s = " a c t i o n - l i n k "   o n c l i c k = " s t a r t E d i t ( $ { c . i d } ) " > < i   c l a s s = " p h   p h - p e n c i l " > < / i >   D Ã ¼ z e n l e < / s p a n > 
 
                                                                         < s p a n   c l a s s = " a c t i o n - l i n k   d e l e t e "   o n c l i c k = " d e l e t e C o m m e n t ( $ { c . i d } ) " > < i   c l a s s = " p h   p h - t r a s h " > < / i >   S i l < / s p a n > 
 
                                                                 `   :   ' ' } 
 
                                                         < / d i v > 
...
                                                 < / d i v > 
 
                                                 < d i v   c l a s s = " c o m m e n t - b o d y "   i d = " b o d y - $ { c . i d } " > $ { e s c a p e H t m l ( c . c o n t e n t ) } < / d i v > 
 
                                                 < d i v   i d = " e d i t - f o r m - $ { c . i d } "   s t y l e = " d i s p l a y : n o n e ; " > < / d i v > 
 
                                         < / d i v > 
 
                                 < / d i v > 
...
                 / /   S e t u p   F o r m 
 
                 c o n s t   f o r m C o n t a i n e r   =   d o c u m e n t . g e t E l e m e n t B y I d ( ' c o m m e n t - f o r m - c o n t a i n e r ' ) ; 
 
                 i f ( f o r m C o n t a i n e r )   { 
 
                         i f   ( t o k e n )   { 
 
                                 f o r m C o n t a i n e r . i n n e r H T M L   =   ` 
 
                                         < t e x t a r e a   i d = " c o m m e n t - i n p u t "   p l a c e h o l d e r = " D Ã ¼ Å xÃ ¼ n c e l e r i n i z i   p a y l a Å xÄ ± n . . . " > < / t e x t a r e a > 
 
                                         < b u t t o n   c l a s s = " b t n - p r i m a r y "   o n c l i c k = " p o s t C o m m e n t ( ) "   s t y l e = " f l o a t :   r i g h t ; " > Y o r u m   Y a p < / b u t t o n > 
 
...
                         }   e l s e   { 
 
                                 f o r m C o n t a i n e r . i n n e r H T M L   =   ' < p   c l a s s = " l o g i n - p r o m p t " > Y o r u m   y a p a b i l m e k   i Ã § i n   < a   h r e f = " # "   o n c l i c k = " o p e n M o d a l ( \ ' l o g i n M o d a l \ ' ) " > g i r i Å x  y a p m a l Ä ± s Ä ± n Ä ± z < / a > . < / p > ' ; 
 
                         } 
 
                 } 
...
 
 
 a s y n c   f u n c t i o n   p o s t C o m m e n t ( )   { 
 
         c o n s t   c o n t e n t   =   d o c u m e n t . g e t E l e m e n t B y I d ( ' c o m m e n t - i n p u t ' ) . v a l u e ; 
 
         i f   ( ! c o n t e n t   | |   ! c o n t e n t . t r i m ( ) )   r e t u r n   s h o w T o a s t ( ' L Ã ¼ t f e n   b i r   y o r u m   y a z Ä ± n ' ,   ' e r r o r ' ) ; 
 
 
 
         c o n s t   p a r a m s   =   n e w   U R L S e a r c h P a r a m s ( w i n d o w . l o c a t i o n . s e a r c h ) ; 
...
         t r y   { 
 
                 c o n s t   r e s   =   a w a i t   f e t c h ( ` $ { A P I _ U R L } / a r t i c l e s / $ { i d } / c o m m e n t s ` ,   { 
 
                         m e t h o d :   ' P O S T ' , 
 
                         h e a d e r s :   { 
...
                         } , 
 
                         b o d y :   J S O N . s t r i n g i f y ( {   c o n t e n t   } ) 
 
                 } ) ; 
 
 
...
                 i f   ( r e s . o k )   { 
 
                         s h o w T o a s t ( ' Y o r u m u n u z   g Ã ¶ n d e r i l d i !   E d i t Ã ¶ r   o n a y Ä ± n d a n   s o n r a   y a y Ä ± n l a n a c a k t Ä ± r . ' ,   ' s u c c e s s ' ) ; 
 
                         d o c u m e n t . g e t E l e m e n t B y I d ( ' c o m m e n t - i n p u t ' ) . v a l u e   =   ' ' ; 
 
                         l o a d C o m m e n t s ( i d ) ; 
 
                 }   e l s e   { 
 
                         c o n s t   d a t a   =   a w a i t   r e s . j s o n ( ) ; 
 
                         s h o w T o a s t ( d a t a . m e s s a g e   | |   ' H a t a   o l u Å xt u ' ,   ' e r r o r ' ) ; 
 
                 } 
 
         }   c a t c h   ( e )   {   c o n s o l e . e r r o r ( e ) ;   } 
...
 
 
 f u n c t i o n   s t a r t E d i t ( i d )   { 
 
         c o n s t   b o d y E l   =   d o c u m e n t . g e t E l e m e n t B y I d ( ` b o d y - $ { i d } ` ) ; 
 
         c o n s t   c u r r e n t C o n t e n t   =   b o d y E l . i n n e r T e x t ; 
 
         c o n s t   f o r m C o n t a i n e r   =   d o c u m e n t . g e t E l e m e n t B y I d ( ` e d i t - f o r m - $ { i d } ` ) ; 
 
 
 
         b o d y E l . s t y l e . d i s p l a y   =   ' n o n e ' ; 
 
         f o r m C o n t a i n e r . s t y l e . d i s p l a y   =   ' b l o c k ' ; 
 
         f o r m C o n t a i n e r . i n n e r H T M L   =   ` 
 
                 < t e x t a r e a   c l a s s = " e d i t - a r e a "   i d = " e d i t - i n p u t - $ { i d } "   r o w s = " 3 " > $ { e s c a p e H t m l ( c u r r e n t C o n t e n t ) } < / t e x t a r e a > 
 
                 < d i v   s t y l e = " m a r g i n - t o p :   8 p x ;   t e x t - a l i g n :   r i g h t ; " > 
 
                         < b u t t o n   c l a s s = " b t n   b t n - l o g i n "   s t y l e = " p a d d i n g :   6 p x   1 2 p x ;   f o n t - s i z e :   0 . 8 r e m ; "   o n c l i c k = " c a n c e l E d i t ( $ { i d } ) " > Ä ° p t a l < / b u t t o n > 
 
                         < b u t t o n   c l a s s = " b t n   b t n - s i g n u p "   s t y l e = " p a d d i n g :   6 p x   1 2 p x ;   f o n t - s i z e :   0 . 8 r e m ; "   o n c l i c k = " s a v e E d i t ( $ { i d } ) " > K a y d e t < / b u t t o n > 
 
                 < / d i v > 
 
         ` ; 
...
 
 
 f u n c t i o n   c a n c e l E d i t ( i d )   { 
 
         d o c u m e n t . g e t E l e m e n t B y I d ( ` b o d y - $ { i d } ` ) . s t y l e . d i s p l a y   =   ' b l o c k ' ; 
 
         d o c u m e n t . g e t E l e m e n t B y I d ( ` e d i t - f o r m - $ { i d } ` ) . s t y l e . d i s p l a y   =   ' n o n e ' ; 
 
 } 
 
 
 
 a s y n c   f u n c t i o n   s a v e E d i t ( i d )   { 
 
         c o n s t   c o n t e n t   =   d o c u m e n t . g e t E l e m e n t B y I d ( ` e d i t - i n p u t - $ { i d } ` ) . v a l u e ; 
 
         i f   ( ! c o n t e n t . t r i m ( ) )   r e t u r n   s h o w T o a s t ( ' Y o r u m   b o Å x  o l a m a z ' ,   ' e r r o r ' ) ; 
 
 
 
         c o n s t   t o k e n   =   l o c a l S t o r a g e . g e t I t e m ( ' t o k e n ' ) ; 
...
         t r y   { 
 
                 c o n s t   r e s   =   a w a i t   f e t c h ( ` $ { A P I _ U R L } / c o m m e n t s / $ { i d } ` ,   { 
 
                         m e t h o d :   ' P U T ' , 
 
                         h e a d e r s :   { 
...
                 i f   ( r e s . o k )   { 
 
                         s h o w T o a s t ( ' Y o r u m   g Ã ¼ n c e l l e n d i   v e   t e k r a r   o n a y a   g Ã ¶ n d e r i l d i . ' ,   ' s u c c e s s ' ) ; 
 
                         c o n s t   p a r a m s   =   n e w   U R L S e a r c h P a r a m s ( w i n d o w . l o c a t i o n . s e a r c h ) ; 
 
                         l o a d C o m m e n t s ( p a r a m s . g e t ( ' i d ' ) ) ; 
 
                 }   e l s e   { 
 
                         s h o w T o a s t ( ' G Ã ¼ n c e l l e m e   b a Å xa r Ä ± s Ä ± z ' ,   ' e r r o r ' ) ; 
 
                 } 
 
         }   c a t c h   ( e )   {   c o n s o l e . e r r o r ( e ) ;   } 
...
 
         i f   ( ! c o n f i r m ( ' B u   y o r u m u   s i l m e k   i s t e d i Ä xi n i z e   e m i n   m i s i n i z ? ' ) )   r e t u r n ; 
 
         c o n s t   t o k e n   =   l o c a l S t o r a g e . g e t I t e m ( ' t o k e n ' ) ; 
 
         t r y   { 
 
                 c o n s t   r e s   =   a w a i t   f e t c h ( ` $ { A P I _ U R L } / c o m m e n t s / $ { i d } ` ,   {   m e t h o d :   ' D E L E T E ' ,   h e a d e r s :   {   ' A u t h o r i z a t i o n ' :   ` B e a r e r   $ { t o k e n } `   }   } ) ; 
 
                 i f   ( r e s . o k )   { 
 
                         s h o w T o a s t ( ' Y o r u m   s i l i n d i ' ,   ' s u c c e s s ' ) ; 
 
                         d o c u m e n t . g e t E l e m e n t B y I d ( ` c o m m e n t - $ { i d } ` ) . r e m o v e ( ) ; 
 
                 }   e l s e   { 
 
                         s h o w T o a s t ( ' S i l m e   b a Å xa r Ä ± s Ä ± z ' ,   ' e r r o r ' ) ; 
 
                 } 
 
         }   c a t c h   ( e )   {   c o n s o l e . e r r o r ( e ) ;   } 
